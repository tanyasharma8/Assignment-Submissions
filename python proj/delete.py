...
# function declarartion
    ...
    def update(self):
        if self.var_ref.get()=="":
            messagebox.showerror("Error",'Nothing to update')
        else:
            try:
                con=cx_Oracle.connect('scott/tiger')
                cur=con.cursor()
                state="update customer set (:cust_no,:name,:contact,:email,:sex,:post_code,:addhar_no,:city) where cust_no='" +self.var_ref.get()+"'"
                cur.execute(state)
                con.commit()
                cur.close()
                messagebox.showinfo('Success','Successfully deleted')
...
class Room_Booking:
    def __init__(self,root):
        ...
        Bill=Label(frame,text="Total Cost",font=('times new roman',15,'bold'),bg="white")
        ...
        but1=Button(frame,text="ADD",borderwidth=0,font=('times new roman',15,'bold'),fg='white',bg='black')
