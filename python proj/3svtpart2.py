...
#  full name  
        ...
        # self.var_name.config(validate='key',validatecommand=(validate_name,'%P'))  # %P is screen formating
...
# function declarartion
    ...
    def validation(self):
        ...
        # elif self.var_email.get()!=None and self.var_contact.get()!=None:
        #     x=self.checkemail(self.var_email.get())
        #     y=self.checkcontact(self.var_contact.get())
...
class Room_Booking:
    def __init__(self,root):
        ...
        self.bg1=ImageTk.PhotoImage(file=rclass Room_Booking:
    def __init__(self,root):
        self.root=root
        self.root.title('ROOM BOOKING')
        self.root.geometry('1600x900+0+0')
        ...
        self.bg1=ImageTk.PhotoImage(file=r"C:\Users\SAMRIDHI SHARMA\OneDrive\Pictures\Saved Pictures\download.jpg")
        left_lbl=Label(self.root,image=self.bg1)
        left_lbl.place(x=50,y=100,width=470,height=550)
        ...
        # label and entry
        # room number
        cust_id=Label(frame,text="Customer MobileNo",font=('times new roman',15,'bold'),bg="white")
        ...
        self.hotel_table.column("cust_id",width=100)
        self.hotel_table.column("room_id",width=100)
        self.hotel_table.column("checkin",width=100)
        self.hotel_table.column("checkout",width=100)
        self.hotel_table.column("rent",width=100)
    ...
    def get_data(self):
        if self.cust_id_entry.get()=="" or self.roomno_entry.get()=="" or self.Checkin_entry.get()=="" or self.Checkout_entry.get()=="" or self.Bill_entry.get()=="" or self.days_entry.get()=="":                                              
            ...
            try:   
                  con=cx_Oracle.connect('scott/tiger')
                  cur=con.cursor()    
                  var="insert into booking values(:cust_no,:room_id,:checkin,:checkout,:rent,:days)"
                  cur.execute(var,(self.cust_id_entry.get(),self.roomno_entry.get(),self.Checkin_entry.get(),self.Checkout_entry.get(),self.Bill_entry.get(),self.days_entry.get()))
                  con.commit()    
                  self.fetch_data()                
                  cur2=con.cursor()
    ...
    def delete(self):
        if  self.roomno_entry.get()=="":
            ...
            try:
                ...
                cur=con.cursor() 
                # var_ref=input("enter Customer ID which you want to delete:")
                st="delete from book where room_id='"+self.roomno_entry.get()+"' "
                cur.execute(st)
                con.commit()
                cur.close()
                messagebox.showinfo('Success','Successfully deleted')
    ...
    def search(self):
        con=cx_Oracle.connect('scott/tiger')
        cur=con.cursor()
        cur.execute("select * from book where "+str(self.search_var.get())+" LIKE '%"+str(self.txt_search.get())+"%'")
        rows=cur.fetchall()
        if len(rows)!=0:
            self.hotel_table.delete(*self.hotel_table.get_children())
            for i in rows:
        ...
        days_entry=Entry(frame,textvariable=self.var_day,font=('times new roman',15,'bold'))
        days_entry.place(x=80,y=200,width=250)
    ...
    def get_data(self):
        if self.cust_id_entry.get()=="" or self.var_rno.get()=="" or self.var_checkin.get()=="" or self.var_checkout.get()=="" or self.var_bill.get()==""  or self.var_days.get()=="":                                              
            messagebox.showerror("Error","all fields are requires",parent=self.root)
        else:
            try:   
                con=cx_Oracle.connect('scott/tiger')
                cur=con.cursor()    
                var="insert into booking values(:cust_no,:room_id,:checkin,:checkout,:rent,:days)"
                cur.execute(var,(self.cust_id_entry.get(),self.var_rno.get(),self.var_checkin.get(),self.var_checkout.get(),self.var_bill.get(),self.var_days.get()))
                con.commit()    
                self.fetch_data()                
                cur2=con.cursor()
                cur2.execute('select * from booking')
...
#lower frame
    ...
    def additem(self):
        ...
        if self.rent_var.get()=="":
            ...
            #self.tax_var.set(str('Rs.%.2f'%((((sum(self.l))-(int(self.rent_var.get())))*tax)/100)))
