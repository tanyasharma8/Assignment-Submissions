...
# register button
    ...
    def login(self):                                                            
        ...
        elif self.txtuser.get()=="SSSVT" and self.txtpass.get()=="SSSVT":           
...
class hotelmanagement:
    ...
    # Detail Button
        bookbtn=Button(frame_menu,command=self.user_window4,text="ROOM DETAIL",font=("times new romen",15,'bold'),bd=3,relief=RIDGE,fg='Gold',bg='black',activeforeground='black',activebackground='black')
        bookbtn.place(x=35,y=170,width=500,height=60)   
    
    # Record Button
        bookbtn=Button(frame_menu,text="RESTAURANT",command=self.user_window5,font=("times new romen",15,'bold'),bd=3,relief=RIDGE,fg='Gold',bg='black',activeforeground='black',activebackground='black')
...
class Register:
    def __init__(self,root):
        ...
        self.name_entry=Entry(frame,textvariable=self.var_name,font=('times new roman',15,'bold'))
...
# function declarartion
    ...
    def validation(self):
        ...
        elif self.var_post.get()=='' or len(self.var_post.get()!=6):
...
class Room_Booking:
    ...
    def search(self):
        ...
        con=cx_Oracle.connect('scott/tiger')
        cur=con.cursor()
        cur.execute("select * from booking where room_id LIKE '%"+str(self.txt_search.get())+"%'")
        rows=cur.fetchall()
        if len(rows)!=0:
            self.hotel_table.delete(*self.hotel_table.get_children())
            for i in rows:
...
class Resturant:
    def __init__(self,root):
        self.root=root
        self.root.title('HOTEL RESTAURANT')
        self.root.geometry('1600x900+0+0')
